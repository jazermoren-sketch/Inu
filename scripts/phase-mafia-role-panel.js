...
if (!code.includes(marker)) {
  throw new Error('interactionCreate marker not found');
}

if (code.includes('M17_MAFIA_ROLE_PANEL_HANDLER')) {
  console.log('ℹ️ Mafia role panel handler already applied.');
  process.exit(0);
}

const handler = `client.on("interactionCreate", async interaction => {
  // M17_MAFIA_ROLE_PANEL_HANDLER
  if (interaction.isButton() && interaction.customId.startsWith("m17|")) {
    ...
    if (action === "role") {
      ...
      if (game.phase === "NIGHT" && ["mafia", "doctor", "detective"].includes(player.role)) {
        ...
        rows.push(...mafiaTargetRows(game, { type, actorId: player.id }, type === "doctor"));
      }
...
console.log('✅ Mafia role panel interactions enabled.');
console.log('✅ Mafia night actions now work through ephemeral buttons.');
console.log('✅ Mafia role information remains private.');
